...
class ChoicesAdvancedSearch extends Component
{
    public string $uuid;

    // The constructor is identical to the original ChoicesOffline
    ...
    {
        // The HTML is identical to the original, with only one line changed.
        return <<<'HTML'
                                    ...
                                    <label
                                        ...
                                        <div class="w-full py-0.5 min-h-3 content-center text-wrap">
                                            ...
                                            <input
                                                ...
                                                @if($isReadonly() || $isDisabled() || ! $searchable) readonly @else @focus="focus()" @endif
                                                @if($isDisabled()) disabled @endif
                                             />
                                        </div>
                                   ...
                                   @endif
                                    ...
                                    <div x-ref="choicesOptions">
                                        @foreach($options as $option)
                                            {{-- [THE ONLY CHANGE IS HERE] We combine the label and sub-label for the search value --}}
                                            <div
                                                ...
                                                search-value="{{ data_get($option, $optionLabel) }} {{ $optionSubLabel ? data_get($option, $optionSubLabel) : '' }}"
                                                class="border-s-4 border-base-content/10 focus:bg-base-200 focus:outline-none"
                                                tabindex="0" >
                                                @if($item)
                                                    {{ $item($option) }}
                                                @else
                                                    <x-ui.list-item-advanced :item="$option" :value="$optionLabel" :sub-value="$optionSubLabel" :avatar="$optionAvatar" />
                                                @endif
                                                @if($selection)
