...
trait HasWeeklyPrices
{
    ...
    /**
     * احسب إجمالي السعر لفترة زمنية معينة بناءً على أسعار الأيام.
     *
     * @param  string|\DateTimeInterface  $startDate  تاريخ البداية
     * @param  string|\DateTimeInterface  $endDate  تاريخ النهاية
     * @param  string  $currency  "egp" | "usd"
     */
    public function totalPriceForPeriod($startDate, $endDate, string $currency = 'egp'): float
    {
        ...
        // اجمع سعر كل يوم في الفترة المحددة (من تاريخ البداية حتى قبل تاريخ النهاية)
        while ($current->lessThan($end)) {
            $price = $this->priceForDay($current, $currency);
            if ($price !== null) {
                $total += $price;
            }
            $current->addDay();
    ...
    /**
     * رجّع تفاصيل كل يوم مع سعره في الفترة المحددة + الإجمالي النهائي.
     *
     * @param  string|\DateTimeInterface  $startDate  تاريخ البداية
     * @param  string|\DateTimeInterface  $endDate  تاريخ النهاية
     * @param  string  $currency  "egp" | "usd"
     * @return array ['days' => [], 'total' => float, 'currency' => string, 'nights_count' => int]
     */
    public function priceBreakdownForPeriod($startDate, $endDate, string $currency = 'egp'): array
    {
        ...
        while ($current->lessThan($end)) {
            ...
            $days[] = [
                ...
                'day_name' => $current->locale(app()->getLocale())->translatedFormat('l'), // اسم اليوم بالعربي أو الإنجليزي
                'day_name_en' => $current->format('l'), // اسم اليوم بالإنجليزي
                'price' => $price ?? 0,
