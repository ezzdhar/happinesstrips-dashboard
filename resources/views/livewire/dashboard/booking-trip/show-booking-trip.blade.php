                ...
                <div class="bg-gradient-to-br from-primary/5 to-secondary/5 rounded-lg p-4 border-2 border-primary/30">
                    ...
                    <div class="space-y-2 mb-3">
                        <div class="flex justify-between items-center pb-2 border-b border-primary/20">
                            ...
                            <span class="font-semibold">{{ number_format($booking->price, 2) }} {{ strtoupper($booking->currency) }}</span>
                        </div>

                        @if($adults_price > 0)
                            <div class="flex justify-between items-center">
                                <span class="text-sm">{{ $booking->adults_count }} {{ __('lang.adults') }}</span>
                                <span class="font-semibold">{{ number_format($adults_price, 2) }} {{ strtoupper($booking->currency) }}</span>
                            </div>
                        @endif

                        @if($children_breakdown && $booking->children_count > 0)
                            <div class="mt-3 bg-white dark:bg-base-100 p-3 rounded">
                                <p class="font-semibold mb-2 text-sm">{{ __('lang.children') }} ({{ $booking->children_count }}):</p>
                                <div class="space-y-2">
                                    {{-- Free Children --}}
                                    @if(!empty($children_breakdown['free_children']))
                                        @foreach($children_breakdown['free_children'] as $child)
                                            <div class="flex justify-between items-center text-sm">
                                                ...
                                                <span class="font-medium text-success">{{ number_format($child['price'], 2) }} {{ strtoupper($booking->currency) }}</span>
                                            </div>
                                        @endforeach
                                    @endif
                                    ...
                                    @if(isset($children_breakdown['first_child']))
                                        ...
                                        <div class="flex justify-between items-center text-sm">
                                            ...
                                            <span class="font-medium">{{ number_format($child['price'], 2) }} {{ strtoupper($booking->currency) }}</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex justify-between font-semibold text-sm">
                                        ...
                                        <span>{{ number_format($children_breakdown['total_children_price'] ?? $children_price, 2) }} {{ strtoupper($booking->currency) }}</span>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    @if($booking->trip)
                        <div class="p-2 bg-info/10 rounded text-xs text-info mb-3">
                            ...
                            @if($booking->trip->type->value === 'flexible')
                                ...
                                {{ $booking->adults_count + $booking->children_count }} × {{ number_format($booking->price, 2) }} × {{ $booking->nights_count }} {{ __('lang.nights') }}
                            @else
                    ...
                    <div class="pt-3 border-t-2 border-primary">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="text-right">
                                <p class="text-2xl font-bold text-primary">{{ number_format($booking->total_price, 2) }}</p>
