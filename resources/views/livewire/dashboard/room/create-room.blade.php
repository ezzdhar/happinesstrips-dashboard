    ...
    <x-card title="{{ __('lang.add_room') }}" shadow class="mb-3">
        <form wire:submit.prevent="saveAdd">
            <div class="grid grid-cols-1 sm-only:grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <x-choices-offline required label="{{ __('lang.hotel') }}" wire:model="hotel_id" :options="$hotels"
                    single searchable clearable option-value="id" option-label="name"
                ...
                <x-input required label="{{ __('lang.adults_count') }}" wire:model="adults_count" type="number"
                    min="1" placeholder="{{ __('lang.adults_count') }}" icon="o-users" />
                <x-input required label="{{ __('lang.children_count') }}" wire:model.live="children_count"
                    type="number" min="0" placeholder="{{ __('lang.children_count') }}" icon="o-user-group" />
            ...
            <div class="mt-6 flex justify-end gap-2 px-4 pb-4">
                <x-button label="{{ __('lang.cancel') }}" @click="window.location='{{ route('hotels') }}'"
                    wire:loading.attr="disabled" />
                <x-button label="{{ __('lang.save') }}" class="btn-primary" type="submit"
                    wire:loading.attr="disabled" wire:target="saveAdd,images" spinner="saveAdd" />
            </div>
        </form>
    </x-card>
</div>
