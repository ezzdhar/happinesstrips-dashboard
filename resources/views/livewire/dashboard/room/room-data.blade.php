...
<div>
    <x-card title="{{ __('lang.rooms') }}" shadow class="mb-3">
        <x-slot:menu>
            @can('create_room')
                <x-button noWireNavigate label="{{ __('lang.add') }}" icon="o-plus" class="btn-sm btn-primary"
                    link="{{ route('rooms.create') }}" />
            @endcan
        </x-slot:menu>
        <div class="grid grid-cols-2 sm-only:grid-cols-3 md:grid-cols-3 gap-4 mb-6">
            <x-stat title="{{ __('lang.rooms') }}" value="{{ $rooms->total() }}" icon="fas.list" class="border"
                color="text-primary" />
            <x-stat title="{{ __('lang.rooms_active') }}" value="{{ $rooms_active }}" icon="fas.check-circle"
                class="border" color="text-{{ Status::fromValue(Status::Active)->color() }}" />
            <x-stat title="{{ __('lang.rooms_inactive') }}" value="{{ $rooms_inactive }}" icon="fas.times-circle"
                class="border" color="text-{{ Status::fromValue(Status::Inactive)->color() }}" />
        </div>
        <div class="grid grid-cols-1 sm-only:grid-cols-3 md:grid-cols-3 gap-3 mb-3">
            <x-input label="{{ __('lang.search') }}" wire:model.live.debounce="search"
                placeholder="{{ __('lang.search') }}" icon="o-magnifying-glass" clearable />
            <x-select label="{{ __('lang.status') }}" wire:model.live="status_filter"
                placeholder="{{ __('lang.all') }}" icon="o-flag" clearable :options="[
                    ['id' => Status::Active, 'name' => __('lang.active')],
                    ['id' => Status::Inactive, 'name' => __('lang.inactive')],
                ]" />
            <x-choices-offline label="{{ __('lang.hotel') }}" wire:model.live="hotel_id_filter" :options="$hotels"
                single searchable clearable option-value="id" option-label="name" placeholder="{{ __('lang.select') }}"
                        ...
                        @forelse($rooms as $room)
                            <tr class="bg-base-200">
                                ...
                                <th class="text-center text-nowrap">
                                    <x-badge :value="$room->status->title()" class="text-xs bg-{{ $room->status->color() }}" />
                                </th>
                                <th class="text-center">
                                    @if ($room->is_featured)
                                        <x-badge :value="__('lang.yes')" class="text-xs bg-green-500" />
                                    @else
                                        <x-badge :value="__('lang.no')" class="text-xs bg-red-500" />
                                    @endif
                                </th>
                                    ...
                                    <div class="flex gap-2 justify-center">
                                        ...
                                        @can('delete_room')
                                            <x-button icon="o-trash" class="btn-sm btn-ghost"
                                                wire:click="deleteSweetAlert({{ $room->id }})"
                                                wire:loading.attr="disabled"
                                                wire:target="deleteSweetAlert({{ $room->id }})"
                                                spinner="deleteSweetAlert({{ $room->id }})"
