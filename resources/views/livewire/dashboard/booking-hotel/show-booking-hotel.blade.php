    ...
    <x-card title="{{ __('lang.booking_details') }} - {{ $booking->booking_number }}" shadow class="mb-3">
        <x-slot:menu>
            <x-button noWireNavigate label="{{ __('lang.print') }}" icon="o-printer" class="btn-sm btn-success"
                link="{{ route('bookings.hotels.print', $booking->id) }}" target="_blank" />
            @can('update_booking_hotel')
                <x-button noWireNavigate label="{{ __('lang.edit') }}" icon="o-pencil" class="btn-sm text-white btn-primary"
                    link="{{ route('bookings.hotels.edit', $booking->id) }}" />
                <livewire:dashboard.booking-hotel.update-status-booking-hotel :booking="$booking"
                    wire:key="{{ \Illuminate\Support\Str::random(15) }}" />
            @endcan
        </x-slot:menu>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <p class="text-base font-semibold">{{ $booking->price }} {{ strtoupper($booking->currency) }}
                        </p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-600">{{ __('lang.total_price') }}</label>
                        <p class="text-lg font-bold text-primary">{{ $booking->total_price }}
                            {{ strtoupper($booking->currency) }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    @if (isset($pricing['adult_price_per_person']))
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">{{ number_format($pricing['adult_price_per_person'], 2) }}
                    ...
                    <div class="flex justify-between pt-2 border-t">
                        ...
                        <span
                            class="font-bold text-primary">{{ number_format($booking->bookingHotel->adults_price, 2) }}
                            {{ $pricing['currency'] ?? $booking->currency }}</span>
                    </div>
                </div>
            </div>
                ...
                @if (!empty($pricing['children_breakdown']) && count($pricing['children_breakdown']) > 0 && $booking->children_count)
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between pt-2 border-t">
                            ...
                            <span
                                class="font-bold text-secondary">{{ number_format($booking->bookingHotel->children_price, 2) }}
                                {{ $pricing['currency'] ?? $booking->currency }}</span>
                        </div>
                    </div>
                @else
