	...
	<x-card title="{{ __('lang.booking_details') }} - {{ $booking->booking_number }}" shadow class="mb-3">
		<x-slot:menu>
			...
			@can('update_booking_hotel')
				<x-button noWireNavigate label="{{ __('lang.edit') }}" icon="o-pencil" class="btn-sm text-white btn-primary" link="{{ route('bookings.hotels.edit', $booking->id) }}"/>
				<livewire:dashboard.booking-hotel.update-status-booking-hotel :booking="$booking" wire:key="{{\Illuminate\Support\Str::random(15)}}"/>
			@endcan
		</x-slot:menu>
