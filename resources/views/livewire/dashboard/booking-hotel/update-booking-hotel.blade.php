        ...
        <form wire:submit.prevent="update">
            ...
            <div class="border-b pb-4">
                ...
                <div class="grid grid-cols-1 sm-only:grid-cols-2 md:grid-cols-3 gap-4">
                    <x-choices-offline required label="{{ __('lang.user') }}" wire:model="user_id" :options="$users"
                        option-value="id" option-label="name" option-sub-label="phone" single clearable searchable
                        icon="o-user" />
                    <x-select required label="{{ __('lang.currency') }}" wire:model.live="currency"
                        icon="o-currency-dollar" :options="[
                            ['id' => null, 'name' => __('lang.select')],
                            ['id' => 'egp', 'name' => 'EGP'],
                            ['id' => 'usd', 'name' => 'USD'],
                        ]" />

                    <x-choices-offline required label="{{ __('lang.hotel') }}" wire:model.live="hotel_id"
                        :options="$hotels" option-value="id" option-label="name" single clearable searchable
                        icon="o-building-office-2" placeholder="{{ __('lang.select_hotel') }}" />
                </div>
            </div>
            ...
            <div class="border-b mt-2 pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <x-input required label="{{ __('lang.check_in') }}" wire:model.live="check_in" type="date"
                        icon="o-calendar" />
                    <x-input required label="{{ __('lang.check_out') }}" wire:model.live="check_out" type="date"
                        icon="o-calendar" />
                    <x-input required label="{{ __('lang.nights') }}" wire:model="nights_count" type="number"
                        min="1" icon="o-moon" readonly />
                </div>
            </div>
                                ...
                                <div wire:key="room-{{ $room->id }}"
                                    ...
                                    <label class="cursor-pointer">
                                        <div class="flex items-start justify-between mb-2">
                                            ...
                                            <input type="radio" name="room_selection" value="{{ $room->id }}"
                                                wire:model.live="room_id" class="radio radio-primary" />
                                        </div>
            ...
            @if ($room_id && $selected_room)
                <div class="border-b mt-2 pb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {{-- Adults --}}
                        <div>
                            <x-input required label="{{ __('lang.adults_count') }}" wire:model.live="adults_count"
                                type="number" min="1" :max="$selected_room->adults_count" icon="o-user" />
                        </div>
                                ...
                                <div class="space-y-2 max-h-64 overflow-y-auto">
                                    @foreach ($children_ages as $index => $age)
                                        <div class="flex gap-2 items-center" wire:key="child-age-{{ $index }}">
                                            <x-input label="{{ __('lang.child') }} {{ $index + 1 }}"
                                                wire:model.live="children_ages.{{ $index }}" type="number"
                                                min="0" max="18" hint="{{ __('lang.age') }}"
                                                icon="o-cake" />
                                            <x-button wire:click="removeChild({{ $index }})" icon="o-trash"
                                                class="btn-sm btn-error text-white btn-circle mt-6"
                    ...
                    <div class="mt-4">
                        <x-button wire:click="calculatePrice" class="btn-error btn-sm text-white"
                            spinner="calculatePrice">
                        ...
                        <div class="card-body p-3">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <div class="text-right">
                                    <p class="text-sm">{{ $pricing_result['nights_count'] }} {{ __('lang.nights') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <span class="font-semibold">{{ number_format($pricing_result['adults_total'], 2) }}
                                    {{ $pricing_result['currency'] }}</span>
                            </div>
                                    ...
                                    <div class="space-y-2 bg-base-100 p-3 rounded">
                                        @foreach ($pricing_result['children_breakdown'] as $child)
                                            <div class="flex justify-between items-center text-sm">
                                                <span>{{ __('lang.child') }} {{ $child['child_number'] }}
                                                    ({{ $child['age'] }} {{ __('lang.years') }})</span>
                                        ...
                                        <div class="flex justify-between font-semibold">
                                            ...
                                            <span>{{ number_format($pricing_result['children_total'], 2) }}
                                                {{ $pricing_result['currency'] }}</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="flex justify-between items-center text-xl font-bold">
                                ...
                                <span class="text-primary">{{ number_format($pricing_result['total_price'], 2) }}
                                    {{ $pricing_result['currency'] }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    @foreach ($travelers as $index => $traveler)
                        <div class="bg-base-200 p-4 rounded-lg mb-3" wire:key="traveler-{{ $index }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <x-input required label="{{ __('lang.full_name') }}"
                                    wire:model="travelers.{{ $index }}.full_name" icon="o-user" />
                                <x-phone-input required label="{{ __('lang.phone') }}"
                                    phoneProperty="travelers.{{ $index }}.phone"
                                    keyProperty="travelers.{{ $index }}.phone_key" />
                                <x-input required label="{{ __('lang.nationality') }}"
                                    wire:model="travelers.{{ $index }}.nationality" icon="o-flag" />
                                <x-input required label="{{ __('lang.age') }}"
                                    wire:model="travelers.{{ $index }}.age" type="number" min="1"
                                    icon="o-hashtag" />
                                <x-select required label="{{ __('lang.id_type') }}"
                                    wire:model="travelers.{{ $index }}.id_type" icon="o-identification"
                                    :options="[
                                        ['id' => null, 'name' => __('lang.select')],
                                        ['id' => 'passport', 'name' => __('lang.passport')],
                                        ['id' => 'national_id', 'name' => __('lang.national_id')],
                                    ]" />
                                <x-input required label="{{ __('lang.id_number') }}"
                                    wire:model="travelers.{{ $index }}.id_number" icon="o-hashtag" />
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="mt-6 flex justify-end gap-2 px-4 pb-4">
                <x-button label="{{ __('lang.cancel') }}" @click="window.location='{{ route('bookings.hotels') }}'"
                    wire:loading.attr="disabled" />
                <x-button label="{{ __('lang.update') }}" class="btn-primary" type="submit"
                    wire:loading.attr="disabled" wire:target="update" spinner="update" />
            </div>
        </form>
    </x-card>
</div>
